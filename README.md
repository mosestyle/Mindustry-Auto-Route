...
Supported imported formats are **MP3, OGG Vorbis, WAV, and FLAC**. On Android, **Import music** uses Mindustry's document picker. Android may provide an opaque document path without the original filename extension, so the importer also recognizes supported tracks from their audio-file signature. Selected files are streamed into a private Mosestyle library and verified before they are added. On desktop, **Open music folder** opens the private library directory directly.
